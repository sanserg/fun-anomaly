...
class WeatherHTTPPreload(BasePreload):
    ...
    def getTurbineMetrics (self, buildings = None):
        '''
        ...
        logging.debug("Getting getTurbineMetrics")
        ...
        for bldg in buildings:
            ...
            for etype in energy_data_options:
                ...
                metrics_value.append(buildings['temperature'])
                metrics_compare_percent.append(buildings['pressure'])
        logging.debug("getTurbineMetrics ")
        metrics_value.append(buildings['temperature'])
        metrics_compare_percent.append(buildings['pressure'])

        return metrics_value, metrics_compare_percent

    def getBuildings (self ):

          logging.debug("Getting list of turbined")
          '''
          header = {}
         ...
         # encoded_body = json.dumps(body).encode('utf-8')
          ...
          '''
          turbines = {"deviceid" : ["A101","B102"],
                        "turbine_model" : ["M101","M102"],
                        "temperature" : [37,39],
                        "pressure" : [92,89]}
          return turbines
    ...
    def execute(self, df, start_ts = None,end_ts=None,entities=None):
        ...
        #rows = len(buildings)
        rows = 3
        logging.debug('rows  count %s ' %rows)
        response_data = {}
        ...
        for m in metrics:
            logging.debug('metric value %s ' %m)
        ...
        metrics_value, metrics_compare_percent = self.getTurbineMetrics ( buildings = buildings)
